...
if (!mapboxgl.accessToken) {
    console.error("CRITICAL ERROR: Missing NEXT_PUBLIC_MAPBOX_TOKEN environment variable. Mapbox map cannot load.");
}
...
export default function Map() {
    ...
    const [geojson, setGeojson] = useState<GeoJSON.FeatureCollection<GeoJSON.Geometry, BaseFeatureProperties> | null>(null);
    ...
    useEffect(() => {
        ...
        const applySupabaseStateToMap = (
            ...
            if (!currentMap || !currentMap.isStyleLoaded()) {
                console.warn("[Apply State] Map not ready or style not loaded, skipping state application.");
                return;
            }
            console.log(`[Apply State] Applying initial state for ${supabaseData.length} segments (using strings)...`);
            ...
            try {
                supabaseData.forEach((segment) => {
                    ...
                    if (typeof featureIdInput === 'string') {
                        ...
                        console.warn(`[Apply State] Skipping segment with invalid osmid: ${featureIdInput}`);
                        return; 
                    }

                    if (isNaN(featureId)) {
                        console.warn(`[Apply State] Skipping segment with NaN osmid after parsing: ${featureIdInput}`);
                        return;
                    }
        ...
        map.on("load", async () => {
            ...
            try {
                ...
                source = currentMap.getSource("traffic_edges") as GeoJSONSource | undefined;
                if (!source) throw new Error("Failed to get source 'traffic_edges' after adding it.");
            ...
            let processedGeoJson: GeoJSON.FeatureCollection<GeoJSON.Geometry, BaseFeatureProperties> | null = null;
            try {
                console.log("[Map Setup] Fetching GeoJSON data (sf_traffic_map.json)...");
                const resp = await fetch("/data/sf_traffic_map.json");
                if (!resp.ok) {
                    throw new Error(`HTTP error fetching GeoJSON! Status: ${resp.status} ${resp.statusText}`);
                }
        ...
        .on<RoadSegment>(
            ...
            if (updatedSegment && mapRef.current && isMapLoadedRef.current) {
                if (updatedSegment && 'osmid' in updatedSegment && 'viz_color' in updatedSegment && mapRef.current && isMapLoadedRef.current) {
                    ...
                    if (!isNaN(featureId)) {
                        console.log(
                            `[Realtime] Updating feature ${featureId}`
                        );
